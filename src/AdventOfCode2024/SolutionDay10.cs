...
public static class SolutionDay10
{
	...
	{
		var trailHeadRatingSummation = 0L;
		...
		for (var y = 0; y < input.Length; y++)
		{
			...
			for (var x = 0; x < line.Length; x++)
			{
				...
				if (height == 0)
				{
					trailHeadRatingSummation += FindRating(new MapPosition(0, x, y), input, maxX, maxY);
				}
			}
		}

		return trailHeadRatingSummation;
	}
	...
	private static int FindRating(MapPosition position, ImmutableArray<string> input, int maxX, int maxY)
	{
		var hikingTrails = new List<HikingTrail>() { new() { position } };
